...
`).split(`
`),i="",n="",o=[];for(;s.length>0;){let a=!1,l=[],c;for(c=0;c<s.length;c++)if(this.rules.other.blockquoteStart.test(s[c]))l.push(s[c]),a=!0;else if(!a)l.push(s[c]);else break;s=s.slice(c);let p=l.join(`
`),g=p.replace(this.rules.other.blockquoteSetextReplace,`
    $1`).replace(this.rules.other.blockquoteSetextReplace2,"");i=i?`${i}
${p}`:p,n=n?`${n}
${g}`:g;let m=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(g,o,!0),this.lexer.state.top=m,s.length===0)break;let k=o.at(-1);if(k?.type==="code")break;if(k?.type==="blockquote"){let w=k,_=w.raw+`
`+s.join(`
`),N=this.blockquote(_);o[o.length-1]=N,i=i.substring(0,i.length-w.raw.length)+N.raw,n=n.substring(0,n.length-w.text.length)+N.text;break}else if(k?.type==="list"){let w=k,_=w.raw+`
`+s.join(`
`),N=this.list(_);o[o.length-1]=N,i=i.substring(0,i.length-k.raw.length)+N.raw,n=n.substring(0,n.length-w.raw.length)+N.raw,s=_.substring(o.at(-1).raw.length).split(`
`);continue}}return{type:"blockquote",raw:i,tokens:o,text:n}}}list(t){let e=this.rules.block.list.exec(t);if(e){let s=e[1].trim(),i=s.length>1,n={type:"list",raw:"",ordered:i,start:i?+s.slice(0,-1):"",loose:!1,items:[]};s=i?`\\d{1,9}\\${s.slice(-1)}`:`\\${s}`,this.options.pedantic&&(s=i?s:"[*+-]");let o=this.rules.other.listItemRegex(s),a=!1;for(;t;){let c=!1,p="",g="";if(!(e=o.exec(t))||this.rules.block.hr.test(t))break;p=e[0],t=t.substring(p.length);let m=e[2].split(`
...
`}hr(t){return`<hr>
`}list(t){let e=t.ordered,s=t.start,i="";for(let a=0;a<t.items.length;a++){let l=t.items[a];i+=this.listitem(l)}let n=e?"ol":"ul",o=e&&s!==1?' start="'+s+'"':"";return"<"+n+o+`>
`+i+"</"+n+`>
...
`}table(t){let e="",s="";for(let n=0;n<t.header.length;n++)s+=this.tablecell(t.header[n]);e+=this.tablerow({text:s});let i="";for(let n=0;n<t.rows.length;n++){let o=t.rows[n];s="";for(let a=0;a<o.length;a++)s+=this.tablecell(o[a]);i+=this.tablerow({text:s})}return i&&(i=`<tbody>${i}</tbody>`),`<table>
<thead>
`+e+`</thead>
`+i+`</table>
...
${g}`;this.addMessage("assistant",m,!0)}s.push({role:"user",content:a})}this.conversation=s.map(i=>({role:i.role,content:typeof i.content=="string"?i.content:JSON.stringify(i.content)}))}addMessage(t,e,s=!1){let i=document.getElementById("chat-messages");if(!i)return;let n=document.createElement("div");if(n.className=`message ${t}`,s){let o=e.split(`
`),a=o.slice(0,2).join(`
`),l=o.length>2;n.innerHTML=`
...
Be concise and helpful. Always use the eval_js tool when the user asks you to interact with the page.`;try{let n=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":this.apiKey,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:this.selectedModel,max_tokens:1e3,system:[{type:"text",text:i,cache_control:{type:"ephemeral"}}],tools:s,messages:t.map((a,l)=>l===t.length-1&&a.role==="user"?{...a,content:typeof a.content=="string"?[{type:"text",text:a.content,cache_control:{type:"ephemeral"}}]:Array.isArray(a.content)?a.content.map((c,p)=>p===a.content.length-1?{...c,cache_control:{type:"ephemeral"}}:c):a.content}:a)})});if(!n.ok)throw n.status===401?(this.handleUnauthorized(),new Error("Invalid API key. Please enter a valid Anthropic API key.")):new Error(`API request failed: ${n.status}`);let o=await n.json();if(o.usage){this.updateTokenUsage(o.usage),this.updateTokenDisplay();let a=this.calculateCost(o.usage,this.selectedModel);console.log(`Request cost: ${this.formatCost(a)}, Total cost: ${this.formatCost(this.calculateCost(this.totalTokenUsage,this.selectedModel))}`)}return o}catch(n){throw n instanceof TypeError&&n.message.includes("fetch")?new Error(`CORS Error: This website (${window.location.hostname}) blocks API calls to Anthropic. This is a security feature. Try using the bookmarklet on a different site, or consider using a browser extension instead.`):n}}async handleToolCall(toolCall){try{switch(toolCall.name){case"eval_js":let code=toolCall.input.code,result=eval(code),resultString=String(result||"Code executed successfully"),maxLength=10*1024;if(resultString.length>maxLength){let t=this._eval_results.length;this._eval_results.push(result);let e=resultString.substring(0,maxLength);return{tool_use_id:toolCall.id,content:`${e}...
